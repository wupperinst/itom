...
This module defines:
    ...
    - the `itom_hub_tinyomo_para` class that replaces a Pyomo ABSTRACT and CONCRETE model definition using classes from tinyomo_para
...
__all__ = ('itom_hub_tinyomo_para',)

# from __future__ import division
import os
import numpy as np
from tinyomo_para import Sets, Set, Params, Param, Vars, Var, Constraints, Constraint, Objective
from tinyomo_para import NonNegativeReals, Reals
from tinyomo_para import write_objective, write_constraints, write_bounds, write_lp
from tinyomo_para import write_variables_overview, write_variables, write_constraints_overview
from tinyomo_para import write_objective_likepyomo, write_constraints_likepyomo, write_bounds_likepyomo, write_lp_likepyomo
from tinyomo_para import write_parameters

import multiprocessing as mp
from tinyomo_para import _build_param_worker  # Import the worker function

##############################################################################

class itom_hub_tinyomo_para(object):
    '''
    ...
    **Public class attributes:**
    ...
    def __init__(self, InputPath=None, OutputPath=None, config=None):
        ...
        # 1. Define parameter specifications as tuples: 
        # (Sets tuple, default value, exchange flag, ParamName)
        param_specs = [
            # Auxiliary
            ((self.TECHNOLOGY, self.MODE_OF_OPERATION), 0, False, 'ModeForTechnology'),
            ((self.TECHNOLOGY, self.PRODUCT), 0, False, 'ProductFromTechnology'),
            ((self.TECHNOLOGY, self.PRODUCT), 0, False, 'ProductToTechnology'),
            ((self.YEAR,), 0, False, 'TimeStep'),

            # Transport Hub
            ((self.LOCATION,), 0, False, 'HubLocation'),
            ((self.TECHNOLOGY,), 0, False, 'HubTechnology'),

            # Global
            ((self.REGION,), 0.05, False, 'DiscountRate'),
            ((self.LOCATION, self.LOCATION, self.PRODUCT, self.TRANSPORTMODE, self.YEAR), 0, True, 'TransportRoute'),
            ((self.LOCATION, self.LOCATION, self.PRODUCT, self.TRANSPORTMODE, self.YEAR), 0.0, True, 'TransportCapacity'),
            ((self.TRANSPORTMODE,), 0, False, 'MultiPurposeTransport'),
            ((self.REGION, self.LOCATION), 0, False, 'Geography'),
            ((self.REGION,), 1, False, 'DepreciationMethod'),

            # Demands
            ((self.REGION, self.PRODUCT, self.YEAR), 0, False, 'Demand'),

            # Performance
            ((self.TRANSPORTMODE,), 1, False, 'TransportCapacityToActivity'),
            ((self.REGION, self.TECHNOLOGY), 1, False, 'CapacityToActivityUnit'),
            ((self.REGION, self.TECHNOLOGY, self.YEAR), 1, False, 'AvailabilityFactor'),
            ((self.REGION, self.TECHNOLOGY), 1, False, 'OperationalLife'),
            ((self.LOCATION, self.TECHNOLOGY, self.YEAR), 0, False, 'LocalResidualCapacity'),
            ((self.REGION, self.TECHNOLOGY, self.PRODUCT, self.MODE_OF_OPERATION, self.YEAR), 0, False, 'InputActivityRatio'),
            ((self.REGION, self.TECHNOLOGY, self.PRODUCT, self.MODE_OF_OPERATION, self.YEAR), 0, False, 'OutputActivityRatio'),

            # Costs
            ((self.REGION, self.TECHNOLOGY, self.YEAR), 0, False, 'CapitalCost'),
            ((self.REGION, self.TECHNOLOGY, self.MODE_OF_OPERATION, self.YEAR), 0, False, 'VariableCost'),
            ((self.REGION, self.TECHNOLOGY, self.YEAR), 0, False, 'FixedCost'),
            ((self.REGION, self.TRANSPORTMODE, self.YEAR), 0.0, False, 'TransportCostByMode'),
            ((self.REGION, self.REGION, self.TRANSPORTMODE, self.YEAR), 0.0, False, 'TransportCostInterReg'),

            # Capacity Constraints
            ((self.REGION, self.TECHNOLOGY, self.YEAR), self.HighMaxDefault, False, 'TotalAnnualMaxCapacity'),
            ((self.REGION, self.TECHNOLOGY, self.YEAR), 0, False, 'TotalAnnualMinCapacity'),

            # Investment Constraints
            ((self.LOCATION, self.TECHNOLOGY, self.YEAR), self.HighMaxDefault, False, 'LocalTotalAnnualMaxCapacityInvestment'),
            ((self.LOCATION, self.TECHNOLOGY, self.YEAR), 0, False, 'LocalTotalAnnualMinCapacityInvestment'),

            # Activity Constraints
            ((self.REGION, self.TECHNOLOGY, self.YEAR), self.HighMaxDefault, False, 'TotalTechnologyAnnualActivityUpperLimit'),
            ((self.REGION, self.TECHNOLOGY, self.YEAR), 0, False, 'TotalTechnologyAnnualActivityLowerLimit'),
            ((self.REGION, self.TECHNOLOGY), self.HighMaxDefault, False, 'TotalTechnologyModelPeriodActivityUpperLimit'),
            ((self.REGION, self.TECHNOLOGY), 0, False, 'TotalTechnologyModelPeriodActivityLowerLimit'),

            # Emissions & Penalties
            ((self.REGION, self.TECHNOLOGY, self.EMISSION, self.MODE_OF_OPERATION, self.YEAR), 0, False, 'EmissionActivityRatio'),
            ((self.REGION, self.EMISSION, self.YEAR), 0, False, 'EmissionsPenalty'),
            ((self.REGION, self.EMISSION, self.YEAR), 0, False, 'AnnualExogenousEmission'),
            ((self.REGION, self.EMISSION, self.YEAR), self.HighMaxDefault, False, 'AnnualEmissionLimit'),
            ((self.REGION, self.EMISSION), 0, False, 'ModelPeriodExogenousEmission'),
            ((self.REGION, self.EMISSION), self.HighMaxDefault, False, 'ModelPeriodEmissionLimit'),
        ]

        # 2. Package parameters with InputPath and SetsGroup for child processes
        tasks = [
            (sets, default, exchange, param_name, self.InputPath, self.AllSets)
            for sets, default, exchange, param_name in param_specs
        ]

        # 3. Process in parallel across available CPU cores
        num_cores = mp.cpu_count()
        with mp.Pool(processes=num_cores) as pool:
            results = pool.map(_build_param_worker, tasks)

        # 4. Attach built Param objects to `self` and update `self.AllParams.all` in the main process
        for param_name, param_obj in results:
            setattr(self, param_name, param_obj)
            self.AllParams.all.append(param_name)
    ...
    def build_lp(self):
        ...
        with open(os.path.join(os.path.abspath(os.pardir), 'parayomo_log.txt'), 'a') as logfile:
            ...
            print('PARALLELISATION', file=logfile)
